...
import {
  ArrowUpRight, BriefcaseBusiness, ChevronDown, Download, FileText, Globe, MapPin, Phone, Search, Star, UserRound,
} from 'lucide-react'
...
const singularOf = (task: TaskKey, fallback: string) => {
  if (task === 'listing') return 'directory entry'
  if (task === 'pdf') return 'reference'
  return fallback.replace(/s$/, '').toLowerCase()
}
...
const cardBase = 'group block rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] transition duration-500 hover:-translate-y-1 hover:border-[var(--tk-accent)]'
...
export function TaskArchiveView({ task, posts, pagination, category, basePath }: { task: TaskKey; posts: SitePost[]; pagination: SiteFeedPagination; category: string; basePath: string }) {
  ...
  const rawLabel = taskConfig?.label || task
  const label = taskLabelDisplay(task, rawLabel)
    ...
    <EditableSiteShell>
      <main style={taskThemeStyle(task)} className="min-h-screen bg-[var(--tk-bg)] text-[var(--tk-text)]">
        <header className="relative overflow-hidden">
          <div className="pointer-events-none absolute inset-x-0 -top-40 h-96 bg-[radial-gradient(50%_50%_at_50%_0%,var(--tk-glow),transparent_70%)]" />
          <div className="relative mx-auto max-w-[var(--editable-container)] px-5 py-16 sm:px-8 sm:py-24 lg:px-14">
            ...
            <EditableReveal index={1}>
              <h1 className="editable-display mt-6 max-w-3xl text-balance text-4xl font-medium leading-[1.05] tracking-[-0.02em] sm:text-5xl lg:text-[3.5rem]">
              ...
              <EditableReveal index={3}>
                <div className="mt-8 flex flex-wrap gap-2.5">
                  {voice.chips.map((chip) => (
                    <span key={chip} className="rounded-full border border-[var(--tk-line)] bg-[var(--tk-surface)] px-4 py-1.5 text-[11px] font-medium uppercase tracking-[0.14em] text-[var(--tk-muted)]">{chip}</span>
                  ))}
                </div>
              </EditableReveal>
            ) : null}
            ...
            <EditableReveal index={4}>
              <div className="mt-12 flex flex-col gap-4 border-t border-[var(--tk-line)] pt-6 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-sm text-[var(--tk-muted)]">
                  <span className="font-semibold text-[var(--tk-text)]">{posts.length}</span> {posts.length === 1 ? singularOf(task, rawLabel) : `${singularOf(task, rawLabel)}s`} · {categoryLabel}
                </p>
                <form action={basePath} className="flex items-center gap-2.5">
                  ...
                  <button className="btn-pill inline-flex h-11 items-center rounded-full bg-[var(--tk-accent)] px-6 text-sm font-medium text-[var(--tk-on-accent)] transition hover:opacity-90">Apply</button>
                </form>
              </div>
            </EditableReveal>
          </div>
        ...
        {showHeaderAd ? (
          <div className="mx-auto max-w-[var(--editable-container)] px-5 pb-4 sm:px-8 lg:px-14">
            <Ads slot="header" size={pickRandom(getSlotSizes('header'))} showLabel />
          </div>
        ) : null}

        <section className="mx-auto max-w-[var(--editable-container)] px-5 py-16 sm:px-8 sm:py-20 lg:px-14">
          {posts.length ? (
            <div className={taskGrid[task]}>
              {posts.map((post, index) => (
                <EditableReveal key={post.id || post.slug} index={index % 8} as={task === 'image' ? 'div' : 'div'} className={task === 'image' ? 'break-inside-avoid' : ''}>
...
function CardArrow({ label }: { label: string }) {
  return (
    <span className="btn-pill mt-5 inline-flex items-center gap-1.5 text-sm font-medium text-[var(--tk-accent)]">
      {label}
      <ArrowUpRight className="btn-arrow h-4 w-4" />
    </span>
  )
}
...
function ImageArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  const image = getImage(post)
  return (
    <Link href={href} className="group mb-5 block break-inside-avoid overflow-hidden rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] transition duration-500 hover:-translate-y-1 hover:border-[var(--tk-accent)]">
      <div className={`relative overflow-hidden ${index % 3 === 0 ? 'aspect-[3/4]' : 'aspect-[4/3]'}`}>
        ...
        <div className="absolute inset-0 bg-[linear-gradient(180deg,transparent_45%,rgba(42,20,88,0.78))] opacity-90 transition group-hover:opacity-100" />
        <div className="absolute inset-x-0 bottom-0 p-5">
          ...
          <span className="mt-2 inline-flex items-center gap-1.5 text-xs font-medium text-white/80">View image <ArrowUpRight className="h-3.5 w-3.5" /></span>
        </div>
      </div>
    </Link>
  )
  ...
  return (
    <Link href={href} className={`${cardBase} flex gap-4 p-6`}>
      ...
      <div className="min-w-0 flex-1">
        ...
        <h2 className="editable-display mt-1.5 text-lg font-medium leading-snug tracking-[-0.02em]">{post.title}</h2>
...
function resolveFileSizeFromPost(post: SitePost): string {
  const str = getField(post, ['fileSize', 'size', 'sizeLabel'])
  if (str) return str
  const content = getContent(post)
  const num = ['fileSize', 'size', 'bytes', 'fileBytes'].map((k) => Number(content[k])).find((n) => Number.isFinite(n) && n > 0)
  if (num) return formatBytes(num)
...
function PdfArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  const size = resolveFileSizeFromPost(post)
  return (
    <Link href={href} className={`${cardBase} flex flex-col p-6 sm:p-7`}>
      <div className="flex items-start justify-between gap-4">
      ...
      <RatingLine post={post} />
      <p className="mt-3 line-clamp-3 flex-1 text-sm leading-7 text-[var(--tk-muted)]">{getSummary(post)}</p>
      <div className="mt-4 flex flex-wrap gap-2 text-[11px] font-medium uppercase tracking-[0.14em] text-[var(--tk-muted)]">
        <span className="rounded-full bg-[var(--tk-raised)] px-3 py-1">Reference</span>
        {pages ? <span className="rounded-full bg-[var(--tk-raised)] px-3 py-1">{pages} pages</span> : null}
        {size ? <span className="rounded-full bg-[var(--tk-raised)] px-3 py-1">{size}</span> : null}
      </div>
      <span className="btn-pill mt-6 inline-flex items-center gap-1.5 text-sm font-medium text-[var(--tk-accent)]">Open reference <Download className="h-4 w-4" /></span>
    </Link>
  )
}
